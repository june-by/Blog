...
`Lazy Loading`의 경우에는, 리액트에서 제공하는 `React.lazy` 를 이용해서, 쉽게 구현 할 수 있다.
...
여기서 하나의 문제점이 발생했는데, 해당 컴포넌트가 무겁다 보니, `해설 영상 버튼`을 클릭해서 사용자가 실제, 모달 내부의 콘텐츠를 이용 할 수 있을 때까지 **로딩 시간이 길다는 점**이었습니다.
...
const onMouseEnterCommentaryVideoButton = useCallback(() => {
  import("src/utils/youtubeModal/youtube_modal");
}, []);

const onClickCommentaryVideoButton = () =>
...
return (
  <button
    ...
    onClick={onClickCommentaryVideoButton}
    onMouseEnter={onMouseEnterCommentaryVideoButton}
  >
    해설 영상
  </button>
);
