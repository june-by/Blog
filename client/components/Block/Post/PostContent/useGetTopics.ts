...
const useGetTopics = (setTopicArr: React.Dispatch<React.SetStateAction<never[]>>) => {
  useEffect(() => {
    ...
    if ($h1Tags.length !== 0) {
      for (const tag of $h1Tags as unknown as HTMLElement[]) {
        if (tag?.innerText?.length !== 1) tagArr.push(tag);
      }
    }

    if ($h2Tags.length !== 0) {
      for (const tag of $h2Tags as unknown as HTMLElement[]) {
        if (tag?.innerText?.length !== 1) tagArr.push(tag);
      }
    }

    if ($h3Tags.length !== 0) {
      for (const tag of $h3Tags as unknown as HTMLElement[]) {
        if (tag?.innerText?.length !== 1) tagArr.push(tag);
      }
    }
