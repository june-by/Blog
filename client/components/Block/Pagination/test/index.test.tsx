...
describe("<Pagination />", () => {
  const totalPage = 10;
  const router = createMockRouter({ query: { page: "4" }, pathname: "/filter" });
  const queryClient = new QueryClient();

  it("rendering test", async () => {
    renderWithContext(router, queryClient, <Pagination totalPage={totalPage} />);
    expect(screen.getByRole("button", { name: "이전" })).toBeInTheDocument();
    expect(screen.getByRole("button", { name: "다음" })).toBeInTheDocument();
    for (const btn of makeArray(Number(router.query.page), totalPage)) {
      if (btn !== "...") expect(await screen.findByRole("button", { name: String(btn) }));
    }
  });
  ...
  it("prevBtn click test", async () => {
    ...
    expect(router.push).toBeCalledWith(makeNextPath(router.pathname, Number(page) - 1, router.query));
  });
  ...
  it("numberBtn click test", async () => {
    ...
    expect(router.push).toBeCalledWith(makeNextPath(router.pathname, Number(nextPage), router.query));
  });
});
