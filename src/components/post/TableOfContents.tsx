      ...
      <ul className="sticky top-10 ml-12 w-[calc(49vw-500px)] space-y-1.5 border-l-2 pl-5 transition-colors dark:border-gray-700">
        {headings.map(({ text, slug, level }: HeadingType) => (
          <li key={text} className="list-none my-0.5">
            <a
              className={`duration-200 text-sm no-underline text-gray-500 hover:text-gray-900 dark:hover:text-gray-200 inline-block transition-all hover:underline ${
                activeToc === slug ? "activeToc" : ""
              }`}
              data-level={level}
              href={`#${slug}`}
            >
