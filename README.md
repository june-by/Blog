...
###  👨‍💻 다크모드
* 다크 모드의 경우 여러 가지 방식을 사용할 수 있지만, SSR을 사용하고 있기 때문에, 쿠키와 React에서 제공하는 Context API를 활용했습니다.
* 사용자가 Theme Mode를 바꿀 경우, 이를 쿠키에 Set하고 Context API의 저장소에도 저장했습니다.
* 이를 통해, 현재 Theme Mode를 파악할 수 있으며, 페이지 이동 중인 상황에서도 Cookie를 통해 사용자가 선택한 Theme Mode를 사용하는 스켈레톤 컴포넌트를 보여줄 수 있도록 했습니다.
* 배경색과 글자색외에도 Theme Mode에 따라 다르게 보여줘야 하는 컨텐츠들(버튼, 입력창 등)의 경우, ThemeContext에서 가져온 현재 Theme에 따라 각각 다른 style을 갖을 수 있도록 만들었습니다.
```javascript
